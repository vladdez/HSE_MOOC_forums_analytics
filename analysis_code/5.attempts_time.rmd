...
```{r}
wd <- getwd()
assignments <- fread("unzip -cq ../data/assignments.zip")  %>%  mutate(course_item_type_desc = factor(course_item_type_desc), vis_b = ifelse(vis_tw==0, 0, 1)) %>% group_by(course_item_id) %>% mutate(difficulty =  sum(first_attempt == 0) / n()) %>% ungroup()
...
```{r}
tw <- assignments %>% filter(assignment_time < 144)
m1 <- glmer(vis_b ~ assignment_time + course_item_type_desc + (1|course_name), family = "binomial", tw)
summary(m1)
vif(m1)
```
```{r}
...
m1 <- glmer(vis_b ~ attempts + course_item_type_desc + (1|course_name), family = "binomial", tw)
summary(m1)
vif(m1)

```
```{r}
m1 <- glmer(vis_b ~ difficulty + course_item_type_desc + (1|course_name), family = "binomial", assignments)
summary(m1)
vif(m1)
```
