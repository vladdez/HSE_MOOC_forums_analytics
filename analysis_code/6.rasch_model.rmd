...
assignments <- fread("unzip -cq ../data/assignments.zip")  %>% group_by(course_name) %>%
 mutate(course_item_type_desc = factor(course_item_type_desc), vis_before_attempt_tw_b = ifelse(vis_before_attempt_tw==0, 0, 1), vis_tw_b = ifelse(vis_tw==0, 0, 1), vis_before_attempt_tw_b = factor(vis_before_attempt_tw_b)) 
...
table(assignments$course_item_type_desc, assignments$vis_tw_b)
...
coef <- display(mr0)$coef # intercept of m0
...
cat(sprintf("Вероятность  с первой попытки выолнить среднее задание средним студентом - %.f%%.\n", round(invlogit(coef), digits = 2) *100))
cat(sprintf("Эта вероятность будет колебаться между %d%% и %d%%  с одним стандартным отклонением квалификации студента ниже и выше.\n", c_proficieny[1] *100, c_proficieny[2] *100))
cat(sprintf("Эта вероятность будет колебаться между %d%% и %d%%  с одним стандартным отклонением сложности задания ниже и выше.\n", c_difficulty[1] *100, c_difficulty[2] *100))
```
...
df <- 3-1
df
# if results are cheater than chi than they are significant
...
```{r, echo=FALSE}
cat(sprintf("Вероятность, что студент выполнит задание с первой попытки, если это курс не по программированию равна %.2f.\n", invlogit(mrr1$coefficients[1])))
cat(sprintf("Вероятность, что студент выполнит задание с первой попытки, если это курс по программированию - %.2f\n\n", invlogit(mrr1$coefficients[1] + mrr1$coefficients[2])))
```
