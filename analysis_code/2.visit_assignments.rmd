...
```{r}
wd <- getwd()
assignments <- fread("unzip -cq ../data/assignments.zip") %>% mutate(vis_tw_b = ifelse(vis_tw==0, 0, 1), vis_before_attempt_tw_b = ifelse(vis_before_attempt_tw==0, 0, 1),) %>% group_by(course_name)   %>% mutate(course_item_type_desc = factor(course_item_type_desc)) %>% ungroup()
```
