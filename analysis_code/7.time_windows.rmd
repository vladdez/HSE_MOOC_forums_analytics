...
- В оригинальной базе данных 60 курсов. На данный момент не учтены курсы, на которых нет пир-ревью: "stalinism" "complex-variable" "moscow-semantic-school" "intro-to-numerical-analysis" "lexical-semantic-typology" "general-relativity" "modular-forms-jacobi" "financial-instruments-capstone".
- Для расчета некоторых курсов не хватило вычислительной мощности: "python-basic" "python-osnovy-programmirovaniya"
...
### Переменные
  ...
  - **vis_tw** - Посещения форума. Сколько раз этот студент посещал форум во время прохождения этого задания;
...
tw_paired %>% group_by(course_item_type_desc) %>% summarise(attempts_vis_tw_mean = mean(attempts_median))
...
- При решении 97% всех заданий студенты посещают форум 1-25 раз, поэтому все значения выше 25 попыток считаем как 11 посещений. Это делается для того, чтобы исключить влияние сверхбольших значений, которые возникли, скорее всего, из-за технических ошибок.

```{r}
tw_paired_0 <- assignments %>%
  mutate(n = n_distinct(course_item_type_desc)) %>% filter(n == 2)
sum(table(tw_paired_0$attempts)[1:11]) / nrow(tw_paired_0)
tw_paired_1 <- tw_paired_0  %>% mutate(vis_tw = case_when(
    vis_tw < 25 ~ vis_tw,
    vis_tw >= 25 ~ as.integer(25)
))
```
...
```{r}
tw_paired <- tw_paired_1  %>%  group_by(course_name, course_item_type_desc) %>% summarise(vis_tw_mean = round(mean(vis_tw), digits = 1))
tw_paired %>% group_by(course_item_type_desc) %>% summarise(vis_tw_mean = round(mean(vis_tw_mean), digits = 1))
...
```{r}
tw_paired  %>% ggplot(aes(x = course_item_type_desc, y = vis_tw_mean)) + geom_boxplot() + geom_point()
...
```{r}
wilcox.test(vis_tw_mean ~ course_item_type_desc, tw_paired, exact = FALSE)
```
...
tw_paired <- assignments %>%
  mutate(n = n_distinct(course_item_type_desc)) %>% filter(n == 2) %>% mutate(vis_tw_b = case_when(
    vis_tw != 0 ~ 1,
    vis_tw == 0 ~ 0
...
tw_paired %>% group_by(course_item_type_desc) %>% summarise(attempts_vis_tw_median = median(attempts_median))
...
tw_paired_1 <- tw_paired_0  %>% filter(vis_tw < 25)

tw_paired <- tw_paired_1  %>%  group_by(course_name, course_item_type_desc) %>% summarise(vis_tw_mean = round(mean(vis_tw), digits = 1))
tw_paired %>% group_by(course_item_type_desc) %>% summarise(vis_tw_median = round(median(vis_tw_mean), digits = 1))
...
```{r}
wilcox.test(vis_tw_mean ~ course_item_type_desc, tw_paired, exact = FALSE)
tw_paired %>% ungroup() %>% wilcox_effsize(vis_tw_mean ~ course_item_type_desc)
```
...
```{r}
wilcox.test(rate ~ course_item_type_desc, tw_paired)
tw_paired %>% ungroup() %>% wilcox_effsize(rate ~ course_item_type_desc)
```
