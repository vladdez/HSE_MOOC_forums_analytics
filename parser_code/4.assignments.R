...
links_in_window_prog <- function(){
  ...
  for (i in 1:99) {
    if (names$ready[i] == "done"){
      ...
      if (i != 6 && nrow(programming_submissions) != 0){
        print(course_name)
  ...
  for (i in 1:99) {
    if (names$ready[i] == "done"){
      ...
      if (file.exists(file) != TRUE){
        ...
        if (i != 6 && nrow(assessment_actions) != 0 && file.exists(paste("/home/vovan/Desktop/visit_ts/",  course_name, ".csv", sep = "")) == TRUE ) {
          ## 1  
          ...
          all <- right_join(assessment_responses, assessment_actions) %>% 
