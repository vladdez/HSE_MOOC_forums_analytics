...
```{r}
prob0<- invlogit(m1$coefficients[1])
prob1<- invlogit(m1$coefficients[1] + m1$coefficients[2])
prob2<- invlogit(m1$coefficients[1] + m1$coefficients[2] * 50)
```
...
cat(sprintf("Вероятность получить высокую оценку студентом, который ни разу не посещал форум: %.4f.\n", prob0))
cat(sprintf("Вероятность получить высокую оценку студентом, который один раз посещал форум: %.4f. \n", prob1))
...
odds2 <- prob2/(1-prob2)
ratio = odds1/odds0
# похожие значения можно получить используя exp(m1$coefficients)
ratio_CI <- exp(confint(m1))
```

```{r , echo=FALSE}

cat(sprintf("Шанс получить высокую оценку если не посещал форум: %.4f к 1.\n", odds0))
cat(sprintf("Шанс получить высокую оценку после посещения одной страницы форума: %.4f к 1.\n", odds1))
cat(sprintf("Отношение шансов = %.5f. \nСледовательно, на каждую посещенную страницу форума шанс получить высокую оценку увеличивается в %.5f (%.5f, %.5f) раз.\n", ratio, ratio, ratio_CI[2], ratio_CI[1]))
cat(sprintf("Например, если студент посетил 50 страниц на форуме, то его шанс получить высокую оценку будет: %.4f к 1.\n", odds2))
```
...
```{r, echo=FALSE}
students$grade_dum <- relevel(students$grade_dum, "0")
students %>% ggplot(aes(y = as.numeric(grade_dum) - 1, x =  vis_p)) + geom_point()  +
  geom_smooth(method = "glm", color = "red", method.args=list(family="binomial")) + labs(y = "Оценка студента", x = "Количество посещенных страниц") + ggtitle("Влияние посещения форума на оценку студента") +  theme(plot.title = element_text(hjust = 0.5))
...
## Выводы
    ...
    1. Студенты которые посещают форум имеет шанс 1.084 к 1 закончить курс на высокую оценку, т.е. оценку больше медианы по курсу.
    2. С каждой посещаемой страницей форума шанс получить оценку выше медианы увеличивается в 1.0004 раза.
