...
```{r , echo=FALSE}
cat(sprintf("С каждым днем выполнения курса количество просмотренных страниц форума  увеличивается в %.4f раз.\n", invlogit(m1$coefficients[2])))
...
```{r , echo=FALSE}
cat(sprintf("Увеличение нормализованного количества посещений форума на 1 пункт удлиняет прохождение курса на %.2f нормализованных дней.\n", invlogit(m1$coefficients[1] + m1$coefficients[2]) - invlogit(m1$coefficients[1])))
cat(sprintf("Человек, который совершил 16 нормализованных посещений форума на %.2f нормализованных дней позже закончил курс.\n ", (invlogit(m1$coefficients[1] + m1$coefficients[2]) - invlogit(m1$coefficients[1])) * 16))
...
## Выводы
    ...
    2. Увеличение нормализованного количества посещений форума на 1 пункт удлиняет прохождение курса на 0.01 нормализованных дней.
