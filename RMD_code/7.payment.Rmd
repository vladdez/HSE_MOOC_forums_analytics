...
```{r , echo=FALSE}
cat(sprintf("Ожидаемое количество страниц просмотренных на форуме у тех, кто не платил за курс: %.2f.\n", exp(m0$coefficients[1])))
cat(sprintf("Ожидаемое количество страниц просмотренных на форуме у тех, кто платил за курс: %.2f.\n", exp(m0$coefficients[1]) + m0$coefficients[2]))
cat(sprintf("Ожидаемое количество страниц посещенных страниц форуме у тех, кто платил за курс выше в %.2f раз чем у тех, кто не платил за курс.\n", exp(m0$coefficients[2])))
...
```{r}
students %>%
  ggplot(aes(y = was_pay, x = vis_p_n)) + geom_point() +  geom_smooth(method="glm.nb", color = "red") + ggtitle("Влияние посещения форума на длительность прохождения курса") + labs(x="Нормализованные посещения", y = "Вероятность оплатить курс") +  theme(plot.title = element_text(hjust = 0.5))
```
...
```{r , echo=FALSE}
cat(sprintf("Ожидаемое нормализованное количество страниц просмотренных на форуме у студентов, которые оплачивали курс выше в %.4f раз чем у тех, кто не оплачивал курс.\n", exp(m1$coefficients[2])))
cat(sprintf("Этот эффект статистически значим: x_squared = %.2f, p = %.6f.\n", m$coefficients[2], m$coefficients[13]))

cat(sprintf("Ожидаемое нормализованное количество страниц просмотренных на форуме курсов с заданиями по программированию выше в %.4f раз чем на курсах без заданий по программированию.\n", exp(m1$coefficients[3])))
cat(sprintf("Этот эффект статистически значим: x_squared = %.2f, p = %.6f.\n", m$coefficients[3], m$coefficients[14]))

cat(sprintf("Ожидаемое нормализованное количество страниц просмотренных на форуме курсов по программированию у студентов, которые оплачивали курс выше в %.4f раз чем на курсах без заданий по программированию и у тех кто не оплачивал курс.\n", exp(m1$coefficients[4])))
...
```{r , echo=FALSE}
cat(sprintf("Ожидаемое количество страниц просмотренных на форуме у тех, кто оплачивал курс по финансовой помощи, выше в %.2f раз чем у тех, кто не оплачивал курс.\n", exp(m1$coefficients[2])))
cat(sprintf("Этот эффект статистически значим: x_squared = %.2f, p = %.6f.\n", m$coefficients[2], m$coefficients[13]))

cat(sprintf("Ожидаемое количество страниц просмотренных на форуме у тех, кто оплачивал курс по групповой помощи, выше в %.2f раз чем чем у тех, кто не оплачивал курс.\n", exp(m1$coefficients[3])))
cat(sprintf("Этот эффект статистически значим: x_squared = %.2f, p = %.6f.\n", m$coefficients[3], m$coefficients[14]))

cat(sprintf("Ожидаемое количество страниц просмотренных на форуме на курсах по программированию у тех, кто оплачивал курс самостоятельно, выше в %.2f раз чем у тех, кто не оплачивал курс.\n", exp(m1$coefficients[4])))
cat(sprintf("Этот эффект статистически значим: x_squared = %.2f, p = %.6f.\n", m$coefficients[4], m$coefficients[15]))
```
